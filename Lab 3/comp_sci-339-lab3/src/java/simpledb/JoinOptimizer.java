...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // ???
            // might be too good to be true. 2.2.2 gives formula
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        // joinOps for equality are Equals Like NotEquals
        ...
        if(joinOp == Predicate.Op.EQUALS || joinOp == Predicate.Op.LIKE || joinOp == Predicate.Op.NOT_EQUALS) {
            ...
            if(t1pkey && !t2pkey) {
                ...
                if(!t1pkey && t2pkey) {
                    ...
                    // could do entire thing with one large statement, but this one just looks pretty
                    // if both have key, should be minimum of the two cardinalities
                    // no need to check t2 this way, since guaranteed to be false
                    card = !t1pkey ? Math.max(card1, card2) : Math.min(card1, card2);
                }
            }
        }
        else {
            // do range scan, 30% fixed fraction as specified
            card = (int) (0.3 * card1 * card2);
        }
        // I have no idea what this means, but hey, written for me
        // var = A (boolean) ? B (value) : (C ? D : E)
        // "var = B if A else C" in python
        // thank u
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        // confusing, instructions say to implement
        // will implement anyway, looks like it is just based on pseudocode
        ...
        Set<LogicalJoinNode> setfinal = null;
        for (int i = 1; i <= this.joins.size(); i++) {
            for (Set<LogicalJoinNode> ljnset : enumerateSubsets(this.joins, i)) {
                int cd = 0;
                Vector<LogicalJoinNode> planfinal = new Vector<>();
                double costfinal = 999999999999.9;
                Iterator<LogicalJoinNode> ljnsetiterator = ljnset.iterator();
                while (ljnsetiterator.hasNext()) {
                    LogicalJoinNode ljn = ljnsetiterator.next();
                    CostCard cc = computeCostAndCardOfSubplan(stats, filterSelectivities, ljn, ljnset, costfinal, pc);
                    // avoid null? from page 6 of lab3 pdf
                    if(cc != null) {
                        costfinal = cc.cost;
                        planfinal = cc.plan;
                        cd = cc.card;
                ...
                pc.addPlan(ljnset, costfinal, cd, planfinal);
                if (i == this.joins.size()) setfinal = ljnset;
            }
        }
        if(explain) printJoins(this.joins, pc, stats, filterSelectivities);
        if(pc.getOrder(setfinal) != null) return pc.getOrder(setfinal);
